...
let sumSquares = 0;
for (let i = 0; i < dataArray.length; i++) { const normalized=(dataArray[i] - 128) / 128; sumSquares +=normalized *
  normalized; } const rms=Math.sqrt(sumSquares / dataArray.length); let decibels=20 * Math.log10(rms); // Shift decibels
  to positive values decibels +=100; // Adjust the offset as needed
  decibelLevelDisplay.textContent=`${decibels.toFixed(1)} dB`; const threshold=parseFloat(thresholdInput.value); //
  Check if decibel level is above threshold if (decibels> threshold) {
